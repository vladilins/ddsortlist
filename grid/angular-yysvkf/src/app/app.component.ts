...
export class AppComponent  implements AfterViewInit{
  ...
  dropListDropped(event: CdkDragDrop<string[]>) {
    ...
    if (this.sourceIndex === -1) {
      this.sourceIndex = parseInt(this.source.id.split('candidate_list_')[1], 10);
    }
    const phElement = this.placeholder.element.nativeElement;
    ...
    if (this.source.id === 'matched_list' || this.target.id === 'matched_list') {
      ...
      if (this.target.id === 'matched_list') {
        ...
        while (this.matched.length > 0) {
          if (this.matched[0] === 0) {
            ...
            transferArrayItem(this.matched, this.items, 0, this.sourceIndex + 1);
          }
        }
  ...
  dropListEnterPredicate = (drag: CdkDrag, drop: CdkDropList) => {
    ...
    const dragIndex = this.__indexOf(dropElement.parentElement.children, (this.source ? phElement :
      sourceElement));
